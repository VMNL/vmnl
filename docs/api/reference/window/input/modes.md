...
Configures GLFW's per-window sticky mouse-button latch and inclusion of lock-key state in input
...
Both modes are disabled on a new window. Setters accept a boolean and getters return the stored
...
Both modes belong to one window and follow its platform-thread constraint. Sticky mouse buttons
latch a native press until the next consuming `glfwGetMouseButton` read. VMNL's `MouseState` is
reduced from events and never performs that read, so its queries remain non-consuming and retain
the documented batch transitions independently of this mode.

Lock-key modifier reporting asks GLFW callbacks to include Caps Lock and Num Lock state. VMNL
preserves those bits as `Modifiers::CAPS_LOCK` and `Modifiers::NUM_LOCK` in mouse-button events.
...
GLFW stores both modes per window in backend-independent input state. Actual lock-key bits still
depend on the platform keyboard state delivered with an input event. No Vulkan capability is used.
