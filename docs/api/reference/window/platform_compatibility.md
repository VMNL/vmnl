...
|---|---|---|
| `Context::get_key_name`, `Context::get_scancode_name` | `glfwGetKeyName` | wayland: conditional — VMNL returns None before keyboard readiness. For a known scancode it resolves the named key first, avoiding the bundled GLFW 3.4 Wayland backend error on non-printable keys; unmapped scancodes use the backend query. Printable keys return an owned UTF-8 active-layout name.; x11: conditional — Returns an owned UTF-8 copy of the active-layout key name; may return null when XKB is unavailable.; win32-cocoa: conditional — Returns an owned UTF-8 copy of the active-layout key name.; null: supported — Returns the deterministic built-in printable name used by the Null backend. |
| `Context::get_key_scancode`, `Context::get_scancode_name` | `glfwGetKeyScancode` | all: conditional — Returns the platform-specific scancode for the named key. |
...
| `Window::is_char_mods_polling_enabled`, `Window::set_char_mods_polling` | `glfwSetCharModsCallback` | all: supported — Installs modified-character callback. |
| `Window::is_char_polling_enabled`, `Window::set_char_polling` | `glfwSetCharCallback` | all: supported — Installs character callback. |
...
| `Window::is_lock_key_modifier_reporting_enabled`, `Window::is_raw_mouse_motion_enabled`, `Window::is_sticky_keys_enabled`, `Window::is_sticky_mouse_buttons_enabled` | `glfwGetInputMode` | all: supported — Returns GLFW's stored cursor, sticky key/button, lock-key modifier or raw-motion mode; effective cursor and raw-motion behavior can differ while unfocused or unsupported. |
...
| `Window::set_cursor_mode`, `Window::set_lock_key_modifier_reporting`, `Window::set_raw_mouse_motion`, `Window::set_sticky_keys`, `Window::set_sticky_mouse_buttons` | `glfwSetInputMode` | wayland: conditional — Stores sticky key/button and lock-key modes in GLFW core. VMNL maps requested Hidden mode to GLFW normal mode plus a transparent cursor; disabled/captured become effective when the pointer enters. Raw motion affects relative motion only in disabled mode.; x11: conditional — Stores sticky key/button and lock-key modes in GLFW core. VMNL maps requested Hidden mode to GLFW normal mode plus a transparent cursor; other cursor modes apply disabled/captured confinement while focused. Raw motion affects disabled mode only.; win32: conditional — Stores sticky key/button and lock-key modes in GLFW core. VMNL maps requested Hidden mode to GLFW normal mode plus a transparent cursor; raw motion affects disabled mode only.; cocoa: conditional — Stores sticky key/button and lock-key modes in GLFW core. VMNL maps requested Hidden mode to GLFW normal mode plus a transparent cursor. Captured mode emits GLFW_FEATURE_UNIMPLEMENTED; raw motion is unsupported.; null: supported — Stores sticky key/button, lock-key, effective cursor and raw-motion modes without a physical cursor. VMNL maps requested Hidden mode to GLFW normal mode plus a transparent cursor. |
