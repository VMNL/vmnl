...
|---|---|---|
| `Context::get_key_name`, `Context::get_scancode_name` | `glfwGetKeyName` | wayland: conditional — VMNL returns None before the first processed keyboard event to avoid the bundled GLFW 3.4 path dereferencing an uninitialized XKB state; later queries return an owned UTF-8 active-layout name.; x11: conditional — Returns an owned UTF-8 copy of the active-layout key name; may return null when XKB is unavailable.; win32-cocoa: conditional — Returns an owned UTF-8 copy of the active-layout key name.; null: supported — Returns the deterministic built-in printable name used by the Null backend. |
| `Context::get_key_scancode` | `glfwGetKeyScancode` | all: conditional — Returns the platform-specific scancode for the named key. |
